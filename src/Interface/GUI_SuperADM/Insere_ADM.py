...
class Cadastro_adm:
    def __init__(self, master=None):
        ...
        self.titulo = Label(self.primeiroContainer, text="Cadastrar Adm")
    ...
    def validar_dados(self):  # valida os campos, se todos estiverem ok, inserir normalmente no arquivo
        if self.cpf.get() == "":  # verificando se todos os campos estão preenchidos
            ...
            resposta = ControladorSuperADM.cadastrar_Adm(adm)  # chamando a função para inserir no arquivo
            if resposta == 'CPF invalido, por favor informar um CPF válido':
                popup_erro(resposta)
            elif resposta == 'Senha inválida!':
                popup_erro(resposta)
            elif resposta == 'Um SuperADM não pode ter o mesmo cpf que um funcionario':
                popup_erro(resposta)
            elif resposta == 'ADM já existe':
                popup_erro(resposta)
